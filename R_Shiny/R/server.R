...
server <- function(input, output, session) {
  ...
  # Increase memory and timeout settings for large files
  # Use gc() to run garbage collection and free memory
  gc(verbose = FALSE)
  
  # Set options for large data handling
  options(shiny.maxRequestSize = 0)  # No size limit (system limits still apply)
  options(future.globals.maxSize = 4096 * 1024^2)  # 4GB limit for future package
  options(shiny.fullstacktrace = TRUE)  # For better error messages
  ...
  observeEvent(input$column_selection_trigger, {
    ...
    # Save current selections based on the current operation (All)
    ...
    } else if (current_operation() == "num_to_cat") {
      previous_selected_cols(input$num_to_cat)
      modal_selected_cols(input$num_to_cat)
    } else if (current_operation() == "cat_to_num") {
      previous_selected_cols(input$cat_to_num)
      modal_selected_cols(input$cat_to_num)
    } else if (current_operation() == "visualization") {
...
if (current_operation() == "missing") {
    ...
    updateSelectInput(session, "num_to_cat", selected = selected_cols)
} else if (current_operation() == "cat_to_num") {
    updateSelectInput(session, "cat_to_num", selected = selected_cols)
}
...
observeEvent(input$modal_select_other, {
  ...
  observeEvent(input$fileInput, {
    ...
    # No file size validation - allow files of any size
  ...
  load_file_data <- function(fileInput) {
    ...
    showNotification("Loading file, please wait...", type = "message", duration = NULL, id = "loading_notification")
    
    tryCatch({
      if (file_ext == "csv") {
        # Use data.table::fread for better performance with large CSV files
        if (!requireNamespace("data.table", quietly = TRUE)) {
          # If data.table is not available, install it
          install.packages("data.table")
          library(data.table)
        }
        
        withProgress(message = 'Reading CSV file...', value = 0, {
          # Use fread for better performance with large files
          data <- data.table::fread(
            fileInput$datapath, 
            data.table = FALSE,  # Return as data.frame
            showProgress = TRUE, # Show progress for large files
            verbose = FALSE,     # Don't print details
            nThread = parallel::detectCores() - 1  # Use multiple cores
          )
          incProgress(1)
        })
        ...
        # Improve Excel reading with progress indication
        ...
        withProgress(message = 'Reading Excel file...', value = 0, {
          # For Excel files, consider options to optimize reading
          data <- read_excel(
            fileInput$datapath, 
            sheet = sheet_names[1],
            guess_max = 1000000  # Increase sample size for column type guessing
          )
          incProgress(1)
        })
      ...
      message("File reading error: ", e$message)
      return(NULL)
    })
  }
  ...
  observeEvent(input$confirm_sheet, {
    ...
    withProgress(message = 'Loading selected sheet...', value = 0, {
      data <- tryCatch({
        read_excel(
...
output$boxplot <- renderPlotly({
 ...
 output$pie_chart <- renderPlot({
  ...
  output$correlation_coefficient <- renderText({
    ...
    if (sum(complete.cases(data.frame(x, y))) < 2) {
      return("Insufficient complete data pairs to calculate correlation")
    }
    ...
    tryCatch({
      ...
      return(paste("Error calculating correlation:", e$message))
    })
  })
  ...
  if(ncol(numeric_data) < 2) {
    ...
    tryCatch({
      ...
      if (nrow(na.omit(numeric_data)) < 2) {
        plot(0, 0, type = "n", axes = FALSE, xlab = "", ylab = "")
        text(0, 0, "Insufficient complete observations for correlation matrix", cex = 1.2)
      } else {
      ...
      plot(0, 0, type = "n", axes = FALSE, xlab = "", ylab = "")
      text(0, 0, paste("Error in correlation matrix:", e$message), cex = 1.2)
    })
  }
})
...
observe({
  ...
  if(any(sapply(data, is.numeric))) {
    numeric_data <- data[, sapply(data, is.numeric), drop = FALSE]
    if(ncol(numeric_data) >= 2 && nrow(na.omit(numeric_data)) >= 2) {
...
output$correlation_matrix_plot <- renderPlot({
  # Skip if no correlation data available
  if (is.null(correlation_data())) {
    plot(0, 0, type = "n", axes = FALSE, xlab = "", ylab = "")
    text(0, 0, "No correlation data available", cex = 1.2)
    return()
  }
  ...
  tryCatch({
    ...
    plot(0, 0, type = "n", axes = FALSE, xlab = "", ylab = "")
    text(0, 0, paste("Error in correlation plot:", e$message), cex = 1.2)
  })
})
          ...
          if (!is.null(input$x_var_bi) && !is.null(input$y_var) &&
            ...
            if (x_var %in% numeric_vars && y_var %in% numeric_vars) {
              tryCatch({
                ...
                tryCatch({
                  if (sum(complete.cases(data.frame(current_data[[x_var]], current_data[[y_var]]))) >= 2) {
